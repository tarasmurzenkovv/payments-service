...
public class PaymentControllerIT extends BaseIT {
    ...
    public void shouldMakePayment() {
        ...
        Assert.assertEquals(response.code(), HttpStatusCode.OK);
        Assert.assertEquals(getAccountBalance(firstAccountId).compareTo(BigDecimal.valueOf(50)), 0);
    }
    ...
    private BigDecimal getAccountBalance(int accountId) {
        ...
        return GenericJsonSerializer.of(response.body().string(), Account.class).getAmount();
    }
    ...
    private int createAccount(int firstCustomerId1, BigDecimal amount) {
        ...
        return GenericJsonSerializer.of(response.body().string(), Account.class).getId();
    }
